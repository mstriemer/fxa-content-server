  ...
  var View = Backbone.View.extend({
     ...
     */
      ...
      if (self._showIndicatorTimeout) {
        ...
        self.removeIndicator(screenName);
        return;
      }
     ...
     */
    removeIndicator: function (screenName) {
      this.$el.remove();

      // Explicitly set the `display: block` using .css. When embedded
      // in about:accounts, the content is not yet visible and must be manually
      // displayed.
      $('#stage').css('display', 'block').addClass('fadein');

      var logo = $('#fox-logo').show().css('opacity', 1);
      if (screenName === 'sign-in' || screenName === 'sign-up') {
        logo.addClass('fade-down-logo');
      }
    },
